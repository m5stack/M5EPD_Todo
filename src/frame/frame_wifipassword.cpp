...
Frame_WifiPassword::Frame_WifiPassword(bool isHorizontal) : Frame_Base()
{
    ...
    if(language == LANGUAGE_JA)
    {
        _key_exit = new EPDGUI_Button("戻る", 272, kKeyBaseY, 260, 52);
        _canvas_title->drawString("パスワード", 270, 34);
    }
    else if(language == LANGUAGE_ZH)
    {
        _key_exit = new EPDGUI_Button("返回", 272, kKeyBaseY, 260, 52);
        _canvas_title->drawString("密码", 270, 34);
    }
    else
    {
        _key_exit = new EPDGUI_Button("Back", 272, kKeyBaseY, 260, 52);
